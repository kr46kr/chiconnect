...
-- 3. Reinforce UNIQUE constraints (in case they were missing or failed due to duplicates)
...
-- Profiles: Only allow users to insert their own profile linked to their auth user_id
DROP POLICY IF EXISTS "Users can insert own profile" ON public.profiles;
CREATE POLICY "Users can insert own profile" ON public.profiles 
FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Event Ratings: Only allow users to manage their own ratings
...
-- Event Attendees: Only allow users to manage their own RSVPs
